...
class ProductController extends Controller
{
    public function index(Request $request){
        if(!\Request::ajax()){
            ...
            // $products = Product::where('status', 1)->orderBy('product_view','desc')->with('brand','product_categories','product_images','product_variations')->paginate(15);
            ...
            //                 ->paginate(15);
            // }
            // $allproducts = cache()->remember('all_products', 60*60*24, function() {
                // return response()->json([
                //     'products' => $products
                // ])
            // });
            $products =  Product::where('status', 1)->orderBy('product_view','desc')->with('brand','product_categories','product_images','product_variations')->paginate(15)->toArray();
    ...
    public function category_product(Request $request){
        ...
        if(!\Request::ajax()){
            ...
            if(isset($category)){
                ...
                if(!empty($productIds)){
                    $products = Product::where('status', 1)->whereIn('id', $productIds)->orderBy('product_view','desc')->with('brand','product_categories','product_images','product_variations')->paginate(15);
                    return response()->json([
                        'products' => $products
                    ], 200);
                }else{
    ...
    public function brand_product(Request $request){
        ...
        if(!\Request::ajax()){
            ...
            $products = [];
            if(isset($brand)){
                $products = Product::where('status', 1)->where('brand_id', $brand->id)->orderBy('product_view','desc')->with('brand','product_categories','product_images','product_variations')->paginate(30);
                    return response()->json([
                        'products' => $products
                    ], 200);
            } else {
    ...
    public function trending_product(Request $request) {

        $trending_products = Product::join('order_details','order_details.product_id','=','products.id')
                        ->select('products.id','products.product_name','products.product_images', DB::raw('count("order_details.id") as total'))
                        ->groupBy('products.id')
                        ->paginate(10);
    ...
    public function addon_product($slug, Product $product)
    {
        ...
        $products = Product::where('status',1)->first()->get();

        foreach($products as $product) {
            $addon_products = DB::table('addon_products')->join('products', 'products.id','=', 'addon_products.selected_product')
                        ->where('addon_products.selected_product', $product->id)
                        ->select('addon_products.addon_products')->get();
                        dd($addon_products);
    ...
    public function singleProductCategory(Request $request){
        ...
        $parent_category = Category::join('product_categories','product_categories.category_id','=','categories.id')
                            ->where('categories.parent_id', 0)
                            ->where('product_categories.product_id', $product->id)
                            ->select('categories.id','categories.category_name', 'categories.cat_slug','categories.meta_title')
                            ->first();
        if(isset($parent_category)){
            $category_name = $parent_category->category_name;
            $cat_slug      = $parent_category->cat_slug;
            $meta_title = $parent_category->meta_title;
        }else{
            $category_name = 'Uncategorized';
            $cat_slug      = '';
        }
        // dd($meta_title, $category_name, $cat_slug);
        return response()->json([
    ...
    public function getViewContent(Request $request){
        $slug = $request->product_slug;
        $product = Product::where('pro_slug', $slug)->select('id','product_sku','regular_price','sales_price')->first();
        $price = 0;

        if($product->sales_price > 0){
            $price = ($product->regular_price) - ($product->sales_price);
        }else{
    ...
    public function brandProductSort(Request $request) {
    ...
    {
        $products = Product::select('id', 'product_name', 'pro_slug', 'featured_image', 'short_description','regular_price', 'sales_price')
        ->where('status', 1)->take(15)->get();
        $brands = Brand::select('id', 'brand_name')->where('status', 1)->get();

        // dd(response()->view('product_feed',[
        //     'products' => $products,
        // ]));
        // $products = Product::all()->take(15);
        // $brand = Brand::all();
        // $brands = $products->brand->name;
        // dd($products, $brands);
    ...
    // public function product_feed_view()
    // {
    //     $products = Product::select('id', 'product_name', 'pro_slug', 'featured_image', 'short_description','regular_price', 'sales_price', '.brand_name')
    //     ->where('status', 1)->get();
    //     $brands = Brand::select('id', 'brand_name')->where('status', 1)->get();

    //     // dd($products);
    //     return response()->view('zapier_test', [
    //        'products' => $products,
    //        'brands' => $brands,
    //     ]);
    // }
