<template>
    <div id="product_edit">
        ...
        <div class="content-wrapper product-edit-area">
            <!-- Main content -->
            <div class="content">
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header border-0">
                                    <h3 class="card-title text-bold">Product Edit</h3>
                                </div>

                                <div class="card-body pt-0">
                                    <el-form ref="form" @submit.prevent="updateProduct()">
                                        <el-row :gutter="20">
                                            ...
                                            <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
                                                <el-form-item label="Brand">
                                                    <el-select v-model="form.brand_id" clearable placeholder="Please Select Brand">
                                        ...
                                        <el-form-item class="image-label" v-if="product.product_images && product.product_images.length > 0">
                                            <label>Previous Image</label>
                                            <el-row :gutter="20">
                                                <el-col class="pl-2 pr-0" :xs="6" :sm="6" :md="2" :lg="2" :xl="2" v-for="(productImage, index) in product.product_images" :key="productImage.id">
                                                    <img :src="`/assets/uploads/product/${productImage.image}`" width="110" height="110" alt="">
                                                    <a href="#" @click="removeImage(index)"><i class="fas fa-times"></i></a>
                                                </el-col>
                                            </el-row>
                                        </el-form-item>
                                        ...
                                        <span v-else>
                                            <!-- Attribute Section -->
                                            <el-row :gutter="20" v-for="(input,k) in new_inputs" :key="k">
                                                <el-col :xs="12" :sm="12" :md="6" :lg="6" :xl="6">
                                                    <el-form-item label="Color">
                                                        <el-select v-model="input.color" placeholder="Please Select Color">
                                                            <el-option v-for="color in allAttributes.colors" :key="color.id" :label="color.attribute_value" :value="color.attribute_value"></el-option>
                                                ...
                                                <el-col :xs="9" :sm="9" :md="4" :lg="4" :xl="4">
                                                    <span>
                                                        <el-form-item class="attr-value">
                                                            <span class="mobile-screen">
                                                                <i class="el-icon-delete" @click="new_remove(k)" v-show="k || ( !k && new_inputs.length > 1)"></i>
                                                                <i class="el-icon-plus" @click="new_add(k)" v-show="k == new_inputs.length-1"></i>
                                                            </span>

                                                            <span class="large-screen">
                                                                <el-button type="danger" icon="el-icon-delete" plain size="small" @click="new_remove(k)" v-show="k || ( !k && new_inputs.length > 1)"></el-button>
                                                                <el-button type="success" icon="el-icon-plus" plain size="small" @click="new_add(k)" v-show="k == new_inputs.length-1"></el-button>
                                                            </span>
                                                        </el-form-item>
                                                    </span>
                                                </el-col>
                                        ...
                                        <fieldset class="product-seo-fieldset">
                                            ...
                                            <el-row class="" :gutter="20">
                                                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                                                    <el-form-item label="Meta Title">
                                                        <el-input v-model="form.meta_title" placeholder="e.g. Angel Water Sipper"></el-input>
                                            ...
                                            <el-row class="" :gutter="20">
                                                <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                                                    ...
                                                    <textarea class="form-control" v-model="form.meta_description" cols="30" rows="5" placeholder="e.g. Meta description"></textarea>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            new_remove (index) {
                this.new_inputs.splice(index, 1)
            },
            removeImage(id){
                this.product.product_images.splice(id, 1);
            },
            categoriesList(){
                this.$store.dispatch('acategory/categoriesList');
            },
            brandsList(){
                this.$store.dispatch('abrand/brandsList')
            },
            getAllOccasion(){
            ...
            sellerShippingList(){
                this.$store.dispatch('ashipping/sellerShippingList')
            },
            updateProduct(){
                let prevImageID=[];
                $.each(this.form.product_images, function(key, value) {
                    prevImageID.push(value.id);
                });
                ...
                if(this.product.product_variations && this.product.product_variations.length > 0){
                    ...
                    $.each(this.new_inputs, function(key, value) {
                        $.each(value, function(Index, Values) {
                            if(Index == 'color') {
                                colors.push(Values);
                            }
                ...
                $('.product-edit-area').css('filter','blur(2px)');
                ...
                formData.append("prev_image_ids", JSON.stringify(prevImageID));
                formData.append("product_images", JSON.stringify(this.form.product_images));
                ...
                formData.append("dimension", this.form.dimension);
                formData.append("voltage", this.form.voltage);
            ...
            getAttributes(){
                this.$store.dispatch('adattribute/getAttributes')
            },
            editProduct(){
                this.$store.dispatch('aproduct/editProduct', this.$route.params.id)
            }
        },
        ...
        computed:{
            ...
            brands(){
                return this.$store.getters['abrand/getAllBrand'];
            },
            occasions(){
            ...
            allAttributes(){
                return this.$store.getters['adattribute/getAttributes'];
            },
            product(){
                return this.$store.getters['aproduct/editProduct'];
            },
            shippings(){
                return this.$store.getters['ashipping/sellerShippingList'];
            },
        },
        ...
        watch:{
            product(){
                ...
                if(this.product.brand_id == 0 || this.product.brand_id == ''){
                    this.form.brand_id = ''
                }
                if(this.product.product_variations){
                    this.inputs = this.product.product_variations
                }
                if(this.product.status == 1){
                    this.status = true
                }
                this.imageShow = '/assets/uploads/product/'+this.product.featured_image
            }
        }
    }
</script>
